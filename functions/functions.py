...
#3. Scan database for matches from list generated by 2
#4. Extend matches to find high scoring alignments
#5. Return all alignments scoring over S
...
def word_score(word,matrix,T):
    ...
    scoreCombs = []
    for j in itertools.product(*scoreVecs):
        scoreCombs.append(j)

    wordCombs = []
    for k in itertools.product(*charVecs):
        wordCombs.append(k)
    
    thresholdWords = []
    for l in range(0,len(scoreCombs)):
